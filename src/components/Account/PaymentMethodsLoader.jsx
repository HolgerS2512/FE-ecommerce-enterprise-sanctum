...
const PaymentMethodsLoader = ({ closeLoader, onClick, user, paymentMeths, btn }) => {
  ...
  const active = renderHTML(find(paymentMeths).byActiveFirst());

  return (
    <>
      <div className="mb-5 mt-225r p fw-semibold">

        {active !== undefined && <div className="mb-1">{t('standard_delivery_payment_methods') + ':'}</div>}

        {active}

        {find(paymentMeths).byNoneActive().map((addr, i) => (
          <div key={i}>
...
export default PaymentMethodsLoader
